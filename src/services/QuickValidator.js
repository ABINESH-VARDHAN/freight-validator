 ...
 * QuickValidator.js
 *
 * Instant, browser-side "document readiness" checker.
 * No AI, no backend calls — everything runs synchronously against the
 * plain text already extracted from a PDF via pdfjs.
 *
 * This is intentionally shallow: it only checks whether the expected
 * fields are PRESENT on the document (regex pattern match), not whether
...
// Try a list of regexes in order, return the first captured value found.
